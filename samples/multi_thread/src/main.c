...
#define FAKE_IDLE_PRIORITY 10
#define NUM_LOOP 10

/* Definizione delle strutture dati per i thread */
K_THREAD_STACK_DEFINE(thread1_stack, STACK_SIZE);
K_THREAD_STACK_DEFINE(thread2_stack, STACK_SIZE);
K_THREAD_STACK_DEFINE(idle_stack, STACK_SIZE);
struct k_thread thread1_data, thread2_data, fake_idle;
...
/*void fake_idle_thread(void *arg1,void *arg2,void *arg3){
	ARG_UNUSED(arg1);
	ARG_UNUSED(arg2);
	ARG_UNUSED(arg3);
	while(end_t);
...
int main(void) {
	...
	/*k_tid_t idle_t = k_thread_create(&fake_idle, idle_stack, STACK_SIZE,
                                   fake_idle_thread, NULL, NULL, NULL,
                                   FAKE_IDLE_PRIORITY, 0, K_NO_WAIT);	
	k_thread_name_set(idle_t,"Idle fake");	*/						   						   
	...
	printf("fine main loop\n");
	end_t = 0;
	k_thread_join(t1,K_FOREVER);
	//k_thread_join(idle_t,K_FOREVER);
	mret_end = read_mret();
